...
Lastly, we introduced a simple prediction-base mechanism for supporting the ability to correct the accuracy internal representation.
...
Then, a prediction error with a specific learning rate is calculated and added to the weight update $\Delta \textbf{W}^{k} = \Delta \textbf{W}^{k} + \hat{\eta}^{k}(\hat{v}^{k}-v^{k}\textbf{u}^{\text{PC}})$.
...
This simple rule aligns with predictive coding \cite{decothiPredictiveMapsRats2022}, and it is inspired by previous computational work on the role of dopaminergic and cholinergic activity in regulating exploration-exploitation behaviours through synaptic plasticity \cite{brzoskoNeuromodulationSpikeTimingDependentPlasticity2019, brzoskoSequentialNeuromodulationHebbian2017}.
