...
The two layers of place cells differ in the size of their place fields. This characteristic is affected by the sensitivity of a cell tuning with respect to the grid cell activation, determined by the parameters of the sigmoid, and the strength of the lateral inhibition, determined by the similarity threshold.
Within a layer, connections between cells are calculated by the same cosine similarity, but compared against a different threshold $\theta^{\text{PC}}_{\text{rec}}$.
...
\begin{figure}[H]
    ...
    \caption{\textsc{Model layout and spatial representations} - \textbf{a}: \textit{the full architecture of the model, consisting of three main sensory input, targeting the two modulators and the cognitive map module, and the executive components, represented by a policy module, two behavioral programs
...
In figure \ref{fig:model}\textbf{d} are shown the centers of one of the fine-grained layers and the activity of two cells are shown, with their place field highlighted as a heat map.
...
In plot \ref{fig:model}\textbf{e} the activity of the two neuromodulators is shown on the fine-grained place cells map, showcasing the limits of the environment and the reward location.
...
Lastly, in order to study the possibility of neuromodulatory alteration of neuronal properties, coarse-grained place cells were subjected to relocation and resizing of their place fields according to neuromodulatory input.
The choice of this layer over the fine-grained one is motivated by empirically better performance results.
...
However, in the case where the agent gets stuck or the distance to the target is shorter than the cells' distance, the planning switches to the other layer for devising a fine-grained plan, which it is followed until either the target or the next node in the coarse-grained are reached.

The advantage of this dual-layer planning lies in its flexibility, as it lightens the computational load of planning by exploding the sparser and lighter map and only invokes the detailed one when necessary.
...
\midrule
Brozsko et al. \cite{brzoskoSequentialNeuromodulationHebbian2017} & One spiking layer & Online modulated Hebbian plasticity & Yes \\
\midrule
