...
ability of rats in a water maze \cite{tolmanCOGNITIVEMAPSRATS1948}.
...
The origin of the spatial tuning of these cells is still debated. Some theories trace it back to competitive dynamics over projections from entorhinal grid cells through the temporo-ammonic pathway, while others point at the Schraffer's collaterals from CA3 \cite{bushWhatGridCells2014,
neherGridCellsPlace2017a, liModelingPlaceCells2019, kubieSpatialFrequenciesGrid2015}.
...
% dopamine
There are several set of dopaminergic innervations to the hippocampus, targeting all its main sub-components. The ones more directly involved with CA1 place cells are those from the ventral tegmental area (VTA) and locus coeruleus (LC) \cite{lismanHippocampalVTALoopControlling2005}.
...
Our primary goal was to investigate the role of neuromodulation in the formation of a cognitive map, and its impact on the agent's behaviour.
The model is constructed with a pre-defined stack of grid cells layers of different scales, from which the place cells were generated through competition \cite{almePlaceCellsHippocampus2014, ormondPlaceFieldExpansion2015}, and a graph-like map is obtained by connecting place cells over one or more trajectories \cite{wernerModellingNavigationalKnowledge2000}.
Previous work has applied neuromodulation in spiking neurons for controlling explorative-exploitative behaviour, inspired by the role of dopamine and acetilcholine \cite{brzoskoRetroactiveModulationSpike2015, brzoskoSequentialNeuromodulationHebbian2017}.
While others used it during training of deep artificial networks \cite{meiEffectsNeuromodulationinspiredMechanisms2023}.
Here, neuromodulators are best described by nodes working as analog sensors of external events. Their role is to form synaptic connections with the place cells, effectively determing a scalar field over their representation.
The result is our operationalization of a cognitive map, a neural structure built online through experience and supporting the necessary information for behaviour.
...
A secondary goal was to enquire about the effect of neuromodulation on the density of place cells, and in particular the reshaping of their place fields following meaningful events, as supported by previous studies \cite{bittnerBehavioralTimeScale2017, milsteinBidirectionalSynapticPlasticity2021}.
