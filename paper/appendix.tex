...
\hfill \break
In our model, each grid cell had a tuning width of 0.04. They were defined 8 modules of size 36, and the relative speed scale were \\ $\{1., 0.8, 0.7, 0.5, 0.4, 0.3, 0.2, 0.1, 0.07\}$.

\subsection{Place cells}

\paragraph{Tuning formation}
% formation | forward + competition
A new place cell's tuning is simply defined as the current GC population vector $\textbf{u}^{\text{GC}}_{t}$, and its index is that of the first silent cell, which gets added to the forward weight matrix $\textbf{W}_{i}^{\text{GC}to \text{PC}}\rightarrow \textbf{u}^{\text{GC}}$.

In order to avoid overlapping of place fields, the tuning process is aborted in case the cosine similarity of the new pattern and the old ones is greter than a threshold $\theta^{\text{PC}}_{\text{rep}}$.

Each cell represents a position in the GC activity space, which can be considered a node within a graph of place cells (PC).
Although it is totally possible to only use the $N^{\text{GC}}$-dimensional tuning patterns and be agnostic about the dimensionality of the space in which the agent lives, to simplify the calculations we mapped each pattern to a 2D positions in a vector space.
Then, the PC recurrent connectivity matrix is calculated with a nearest neighbors algorithm, which instead of a fixed number $K$ of neighbors it uses a distance threshold $\theta^{\text{PC}}_{\text{rec}}$.
...
It is also defined an activity trace, which has an upper value of 1 and decay exponentially:
...
\hfill \break
In the model, two PC populations are defined, which are named \textit{fine}-grained and \textit{coarse}-grained as their main difference were their average place field size, determining the granularity of the place representation.

Below it is reported a table of the parameters specific to the two PC populations.
...
\begin{table}[h]
    ...
    \begin{tabular}{lcccccc}
        ...
        \midrule
        Fine   & 33.0  & 1.0  & 0.86  & 43  & 140 \\
        Coarse & 16.6  & 1.0  & 0.76  & 41  & 24 \\
        \bottomrule
    \end{tabular}
    \caption{\textsc{Comparison of fine and coarse Parameters}}
\end{table}
...
\begin{algorithmic}[1]
    \Require Connectivity matrix $\mathbf{C} \in \{0,1\}^{N^{\text{PC}} \times N^{\text{PC}}}$, Node coordinates $\mathbf{x} \in \mathbb{R}^{N^{\text{PC}} \times 2}$, Node weights $\widetilde{\mathbf{W}} \in \mathbb{R}^N^{\text{PC}}$, Start node $s$, End node $t$
