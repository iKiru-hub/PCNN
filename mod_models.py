...
class RateNetworkSimple:
    ...
    def __init__(self, N: int=1, Nj: int=1, **kwargs):
        ...
        **kwargs: dict
        ...
        # rate function
        rate_func_beta = kwargs.get('rate_func_beta', 3.)
        rate_func_alpha = kwargs.get('rate_func_alpha', 8)
        self.rate_func = lambda x: 1 / (1 + np.exp(-rate_func_beta * (x - rate_func_alpha)))
    ...
    def _update(self, rj: np.ndarray):
        ...
        # Oja rule
        if self._rule == 'oja':
            dWff = self.r * rj.T - self.Wff @ (rj * rj) 
        elif self._rule == 'hebb':
            dWff = self.r * rj.T
    ...
    def step(self, rj: np.ndarray):
        ...
        # update state variables
        self.r = self.rate_func(self.Wff @ rj + self.Wrec @ self.r + \
            self._eps * np.random.randn(self.N, 1))
