...
/*         if (circuits.get_bnd_leaky_v() < 0.001 && \\ */
/*                 space_fine.get_max_activation() > 0.45) { */
/*             space_coarse.update(); */
...
/*             /1* LOG("[Brain] forced exploration : v=" + std::to_string(ssry.get_v())); *1/ */
...
/*         if (goalmd.is_active()) { */
/*             /1* LOG("[Brain] active goal plan"); *1/ */
/*             trg_plan_end = 0; */
/*             bool obstacle = collision > 0.0f; */
/*             std::pair<std::array<float, 2>, bool> progress = \ */
/*                 /1* goalmd.step_plan(obstacle); # <---------------------- *1/ */
/*                     goalmd.step_plan(collision > 0.0f); */
/*         /1* std::pair<std::array<float, 2>, bool> progress = {{0.1f, 0.1f}, false}; *1/ */
/*             // keep going */
/*             if (progress.second) { */
...
/*             forced_exploration = 0; */
/*             /1* LOG("[Brain] end or fail -> random walk"); *1/ */
/*         } */
...
/*         /1* LOG("[brain] action=" + std::to_string(action[0]) + ", " + \ *1/ */
/*         /1*     std::to_string(action[1])); *1/ */
/*         return action; */
/*     } */
