...
class Room:
    ...
    def add_boundary_walls(self):
        # Add walls around the screen boundaries
        thickness = 20
        self.walls.extend([
            Wall(-thickness, -thickness,
                 SCREEN_WIDTH + 2*thickness, thickness),  # Top
            Wall(-thickness, SCREEN_HEIGHT,
                 SCREEN_WIDTH + 2*thickness, thickness),  # Bottom
            Wall(-thickness, -thickness, thickness,
                 SCREEN_HEIGHT + 2*thickness),  # Left
            Wall(SCREEN_WIDTH, -thickness, thickness,
                 SCREEN_HEIGHT + 2*thickness),  # Right
        ])
    ...
    def __call__(self, x: float, y: float, 
        ...
        velocity, collision, new_pos = self.check_collision(x, y, velocity)
        if collision:
            velocity *= self.bounce_coeff
