...
class AgentBody:
    ...
    def __call__(self, velocity: np.ndarray, room) -> Tuple[float, float]:
        ...
        NUM_STEPS = 10  # Increase for more precision, decrease for better performance
        ...
        for _ in range(NUM_STEPS):
            ...
            if collision:
                ...
                x_ok = not any(self.rect.colliderect(wall.rect) for wall in room.walls)
                
                # Try moving in y direction only
                test_pos = prev_pos.copy()
                test_pos[1] += step_velocity[1]
                self.rect.y = int(test_pos[1])
                y_ok = not any(self.rect.colliderect(wall.rect) for wall in room.walls)
...
class RandomAgent:
    def __init__(self, change_interval: int = 30, max_speed: float = 4.0):
