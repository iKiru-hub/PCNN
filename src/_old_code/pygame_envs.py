...
class AgentBody:
    ...
    def __init__(self, brain: object=None,
                 ...
                 color: str=ORANGE, render_freq=100, **kwargs):
        ...
        if self.brain is not None:
            observation = {
                "position": np.array(self._scale_to_01(
                        self.x, self.y)),
                "velocity": self.velocity, #self._scale_to_01_vec(self.velocity),
                "collision": collision
            }
...
def make_room(name: str=None, thickness: int=5,
               bounce_coeff: float=1.0, **kwargs):
