...
class PCNNlayer(mm.minPCNN):
    ...
    def __init__(self, pcnn_params: dict,
        ...
        self._input_layer = it.PlaceLayer(N=self.Nj,
                                          sigma=self.sigma,
                                          bounds=self.bounds)
        # self._input_layer = sc.PlaceCellLayer(
        #                             nx=np.sqrt(self.Nj).astype(int),
        #                             ny=np.sqrt(self.Nj).astype(int),
        #                             sigma=self.sigma,
        #                             bounds=self.bounds)
        logger("{} - sigma={}".format(self._input_layer,
                                      self._input_layer.sigma))
        ...
        self._calc_recurrent_enable = kwargs.get("calc_recurrent_enable", False)
        self._calc_graph_enable = kwargs.get("calc_graph_enable", False)

        # self.nodes = np.zeros((self.N, 2))
        self.edges = []
        self.connectivity = np.zeros((self.N, self.N))

        self.var10 = 0.

    def __repr__(self):
        return "PCNNlayer(N={}, Nj={}, lr={})".format(self.N, self.Nj, self._lr)
    ...
    def __call__(self, x: np.ndarray, frozen: bool=False,
        ...
        x = self._input_layer(x=x)

        # x = x / x.sum()

        self.var10 = (self._Wff@x).max()
        ...
        # centers
        if self._umask.sum() != 0 and self._calc_graph_enable:
    ...
    def __len__(self):
        return self._umask.sum().astype(int)
    ...
    def _calc_average_pos(self, a: np.ndarray) -> np.ndarray:
        ...
        # position as weighted average
        pos = np.array([np.sum(xc * a) / np.sum(a),
                        np.sum(yc * a) / np.sum(a)])

        if pos[0] > 1 or pos[0] < 0:
            # logger.debug("a={}".format(a.flatten()))
            # logger.debug("xc={}\nyc={}".format(xc, yc))
            # logger.debug("nonzero ={}, a.shape={}, xc.shape={}".format(len(self.edges), a.shape, xc.shape))
            # raise ValueError("estimated pos={} over the bounds".format(pos))
            # logger.error("estimated pos={} over the bounds".format(pos))
            return None

        if np.sum(a) == 0:
            logger.error("ao={} [max {}, max_idx {}]".format(ao, ao.max(), ao[self.finite_idx].max()))
        return pos
    ...
    def calc_path(self, x1: np.ndarray, x2: np.ndarray,
        ...
        if src_pos[0] == -np.inf:
            ...
            logger.warning("[centroid] src_pos={}".fomat(src_pos))
        path = [src_pos]
        curr_pos = src_pos.copy()
        logger.debug("src={}, trg={}".format(np.around(src_pos, 2),
                                             np.around(trg_pos, 2)))
        ...
        while exit_condition(distance, min_trg_distance):
            ...
            if np.isnan(drift_move).any():
                logger.debug("drift_a:\n{}".format(drift_a.flatten()))
                logger.debug("drift_pos:\n{}".format(drift_pos.flatten()))
                logger.debug("activation:\n{}".format(curr_a.flatten()))
    ...
    def set_policy(self, policy: object):

        self.policy = policy
        logger("set: {}".format(self.policy))
    ...
    def render(self, use_a: bool=False, new_a: np.ndarray=None,
               alpha: float=0.9, edge_alpha: float=0.9,
               ax: object=None, color: str='grey'):
    ...
    def save_params(self, path: str=None):
        ...
        logger("Parameters saved to {}".format(path))
    ...
    def load_params(self, path: str=None):
        ...
        logger("Parameters loaded from {}".format(path))

    def is_tuned(self, min_n: int=2) -> bool:

        """
        check if the network has been tuned
        """

        return self._umask.sum() >= min_n
...
class Layers:
    ...
    def __init__(self, N: list, S: list, K: list, Ed: list=None,
        ...
        def __repr__(self):
            return "Layers(" + ", ".join(["(N={}, S={})".format(n, s) for \
                n, s in zip(self.N, self.S)]) + ")"
...
class Policy:
    ...
    def __init__(self, lambdas: list=None, isgoal: bool=True,
                 min_dx: float=0.01,
        ...
        ----------
        lambdas: list
            lambda values for the policy.
            Default is None (use defaults)
        isgoal: bool
            if True, the policy will update the lambda
            values based on the distance from the goal.
            Default is True
        speed: float
            speed of the policy.
            Default is 0.01
        **kwargs:
            min_dx: float
                minimum value for the policy vector.
                Default is 0.01
            min_trg_distance: float
                minimum distance to the target.
                Default is 0.01
            strategy: str
                strategy to combine the moves.
                Default is "max"
            k: int
            ...
            threshold: float
                ...
                Default is 0.001
            action_flags: tuple
                flags to update the actions.
                Default is (True, True, True, True)
            actions_defaults: tuple
                default values for the actions.
                Default is (0.8, 0., -1., 0.5, 0.)
        """

        self.min_dx = kwargs.get("min_dx", 0.01)  # not used currently
        ...
        self.action_flags = kwargs.get("action_flags",
                                       (True, True, True, True))
        self._actions_defaults = kwargs.get("actions_defaults",
                                            (0.8, 0., -1., 0.5, 0.))

        # set lambdas
        if lambdas is None:
            self.lambdas = np.array([
                self._actions_defaults[0],
                self._actions_defaults[1]
            ])
        else:
            self.lambdas = lambdas
        self.lambdas_zero = self.lambdas.copy() 
        ...
        def __repr__(self):
            return "Policy(lambdas=({}, speed={}, goal={})".format(
                np.around(self.lambdas, 3), self.speed, self.isgoal)

    def __call__(self, moves: list, distance: float=None) -> np.ndarray:
        ...
        self.current_move = self.current_move / \
            np.linalg.norm(self.current_move) * self.speed
        ...
        self.vector = [
            np.degrees(np.arctan2(self.current_move[1],
                                  self.current_move[0])),
    ...
    def random_move(self):
        ...
        self.current_move = np.random.uniform(-1, 1, 2)
    ...
    def update_parameters(self, action: np.ndarray):

        """
        action: [lambda, beta]
        """

        self.lambdas[0] = action[0] if self.action_flags[0] else \
            self._actions_defaults[0]
        self.lambdas[1] = 1 - action[0] if self.action_flags[1] else \
            self._actions_defaults[1]

        self.beta = action[2] if self.action_flags[2] else \
            self._actions_defaults[2]

        # speed
        self.speed = action[3] * self.speed_base + (1 - action[3]) * \
            self.speed_max if self.action_flags[3] else self.speed_base
...
class MinimalPolicy:
    ...
    """
    Action influencing the policy:
    [0] speed multiplier
    [1] lambda 1 (goal direction)
    [2] lambda 2 (drift direction)
    [3] lambda 3 (random direction)
    """

    def __init__(self, num_lambdas: int=3,
                 speed: float=0.001,
                 min_trg_distance: float=0.01):
        ...
        self.min_trg_distance = min_trg_distance

        # set lambdas
        self.lambdas = np.ones(num_lambdas) / num_lambdas
        self.lambdas_zero = self.lambdas.copy() 
        ...
        # angle action
        self.angle_action = np.random.uniform(0, 2*np.pi)

    def __repr__(self):
        return "MinimalPolicy(lambdas=(" + \
            "{},".format(np.around(self.lambdas, 3)) + \
            " speed={})".format(self.speed)

    def __call__(self, moves: list) -> np.ndarray:

        """
        calculate the policy
        """

        assert len(moves) == 3, "moves must be of length 3"

        # calculate the new move as a weighted sum of the
        # given moves
        lambdas = 0.
        total_move = np.zeros(2)
        for i, (l, m) in enumerate(zip(self.lambdas, moves)):
            if m is not None:
                # self.current_move += l * m
                total_move += l * m
        ...
        # make it an average
        if total_move.prod() != 0:
            self.current_move = total_move / lambdas
        ...
        # check
        if np.isnan(self.current_move).any():
            logger.error("policy_move={} moves={} lambdas={} {}".format(
                self.current_move, moves, lambdas, total_move))
    ...
    def random_move(self):
        ...
        # check
        if np.isnan(self.current_move).any():
            raise ValueError("policy_move={}".format(self.current_move) + \
                " moves={}, distance={}".format(moves, distance))
    ...
    def update_parameters(self, action: np.ndarray):
        ...
        # lambdas
        self.lambdas = action[1:]

        # angle action
        # if action[1] < 0.1:
        #     self.angle_action = np.random.uniform(0, 2*np.pi)
        self.angle_action = action[0] * 2 * np.pi
...
class VelocityPolicy:
    ...
    def __init__(self, speed: float=0.001,
                 min_trg_distance: float=0.01):
    ...
    def __repr__(self):
        return "MinimalPolicy(speed={})".format(self.speed)

    def __call__(self, moves: np.ndarray) -> np.ndarray:

        """
        calculate the policy from the given moves, which are
        a discrete 4 action space:
        0: up; 1: down; 2: left; 3: right
        """

        # calculate velocity from a given move
        # and the speed
        velocity = np.zeros(2)
        velocity[moves%2] = 1 if moves < 2 else -1
...
class Environment:
    ...
    def _calc_reward(self, curr_pos: np.ndarray,
                     trg_pos: np.ndarray) -> float:
...
def make_default_model(N: int=50, Nj: int=12**2,
                       sigma: float=0.005,
                       threshold: float=0.2,
                       ach_threshold: float=0.2,
                       da_threshold: float=0.5,
                       tau_ach: float=100.,
                       tau_da: float=2.,
                       rec_epsilon_exp: float=8):
    ...
    params = {
        ...
        "eq_da": 0.,
        "epsilon": 0.01,
        "rec_epsilon": np.exp(-rec_epsilon_exp),
        "k_neighbors": None,
        "sigma": sigma,
    }
    # *rec_epsilon*
